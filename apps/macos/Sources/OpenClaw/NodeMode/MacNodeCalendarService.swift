...
protocol MacNodeCalendarServicing: Sendable {
    func events(params: OpenClawCalendarEventsParams) async throws -> OpenClawCalendarEventsPayload
    func add(params: OpenClawCalendarAddParams) async throws -> OpenClawCalendarAddPayload
}

final class MacNodeCalendarService: MacNodeCalendarServicing {
    func events(params: OpenClawCalendarEventsParams) async throws -> OpenClawCalendarEventsPayload {
        let store = EKEventStore()
        let status = try await self.ensureEventAccess(store: store, write: false)
        ...
        let payload = Array(store.events(matching: predicate).prefix(limit)).map { event in
    ...
    func add(params: OpenClawCalendarAddParams) async throws -> OpenClawCalendarAddPayload {
        let store = EKEventStore()
        let status = try await self.ensureEventAccess(store: store, write: true)
    ...
    {
        if let id = calendarId?.trimmingCharacters(in: .whitespacesAndNewlines),
           !id.isEmpty,
    ...
    private func ensureEventAccess(store: EKEventStore, write: Bool) async throws -> EKAuthorizationStatus {
        let status = EKEventStore.authorizationStatus(for: .event)
        if write ? EventKitAuthorization.allowsWrite(status: status) : EventKitAuthorization.allowsRead(status: status) {
            return status
        }
